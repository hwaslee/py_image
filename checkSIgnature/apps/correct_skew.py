...
def getSubImage(fullImage, subImge):
	print("")
...
def deskewImage(fullImage, subImage):
	...
	# coordinates
	logging.info("np.where....{}".format(np.where(thresh > 0)))
	...
	if abs(angle) != 0.0:
		...
		deskewedFull = fullImage.copy()
		deskewedFull = cv2.cvtColor(deskewedFull, cv2.COLOR_BGR2GRAY)
		diff = cv2.subtract(fullImage, deskewedFull)
		result = not np.any(diff)
		logging.info("Image compare: result:{}".format(result))
